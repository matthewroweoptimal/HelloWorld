...
uint32_t spi_flash_fw_erase()
{
	...
	{
		result = spi_flash_fw_erase_block(sector);
		if ( result != 1 )
		{
			break;
		}
...
//-----------------------------------------------------------------------------
// Erases a block within the SPI Flash firmware area
//-----------------------------------------------------------------------------
uint32_t spi_flash_fw_erase_block( uint32_t sector )
{
	uint32_t result;
	if ( sector >= SPI_FW_SECTORS )
	    return 0;   // Fail
	    
	uint32_t flashAddress = SPI_FLASH_FW_BASE + sector * SPI_FLASH_FW_SECTOR_SIZE;
	SPIM_EraseBlock( flashAddress,
...
//	printf("\t\tSPI FW Flash Write : Addr 0x%08X, Len %d\n", destAddress, bytesToWrite);
	return( true );	// Always SUCCESS, confirmed on flash verification
}

//-----------------------------------------------------------------------------
// Write Region Header to Start of SPI Flash Firmware area
//-----------------------------------------------------------------------------
bool spi_flash_writeFwHeader( OLY_REGION* pHeader )
{
	SPIM_IO_Write( SPI_FLASH_FW_BASE, USE_4_BYTE_ADDR_MODE, sizeof(OLY_REGION), (uint8_t*)pHeader, OPCODE_PP, 1, 1, 1);
	OLY_REGION verify;
	spi_flash_readFwHeader( &verify );
	return (memcmp(pHeader, &verify, sizeof(OLY_REGION)) == 0);  // return true for successful write
}
...
bool spi_flash_verify_fw_chunk( uint32_t uiFlashOffset, uint8_t* pChunk )
{
	...
	for ( uint32_t i = 0; i < bytesToVerify; i += sizeof(uint32_t), pFlashWord++, pChunkWord++ )
	{
...
//-----------------------------------------------------------------------------
// Read content of FW SPI Flash byte
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Read content of FW SPI Flash word
//-----------------------------------------------------------------------------
...
{	// Firmware content is after the OLY_REGION header.
	...
	SPIM_IO_Read( uiFlashFwAddress, USE_4_BYTE_ADDR_MODE, sizeof(uint32_t), (uint8_t*)&flashContent, OPCODE_FAST_READ, 1, 1, 1, 1);
    return flashContent;
}
