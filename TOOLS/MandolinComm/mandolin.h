...
#define ENABLE_MANDOLIN_TIMEOUT 1 // 0 -> wait (potentially infinitely) for the rest of any message with a valid header,
...
#define MANDOLIN_TIMEOUT               3000  // make it big for now
                                  
#endif

// FIFO Size Switches
#define MANDOLIN_FIFO_IS_POWER_OF_TWO   0      // 0 -> any FIFO size is OK, 1 -> all FIFO sizes must be powers of two
#define MANDOLIN_MAX_PAYLOAD_DATA_WORDS (1024*10) //65535u // 2^16 - 1 data words - the maximum words the current protocol supports
...
#define MANDOLIN_BUFFER_SLOP_BYTES      48u    // The current structure of the FIFO means that we lose the use of one byte,
...
#define MANDOLIN_FIFO_BYTES            (MANDOLIN_MAX_MESSAGE_BYTES*4 + MANDOLIN_BUFFER_SLOP_BYTES)    // MAB - Should be smaller than FIFO_TYPE_MAX/2 to avoid overflow!
...
#define MANDOLIN_FLAG_UNSUBSCRIBE 0
...
typedef enum {
	...
	MANDOLIN_MSG_CUSTOM							= 0x70,
	MANDOLIN_MSG_NESTED							= 0x71,	// payload  target id, target msg  (target=0 invalid)
...
typedef enum {
    ...
    MANDOLIN_ERROR_FLASH_VERIFY 	   = 16,	
...
typedef enum {
    ...
    MANDOLIN_AE_PULL_SHOW,
    MANDOLIN_AE_UPDATE_SHOW,
    MANDOLIN_AE_NAME_SHOW,
...
#define MANDOLIN_MSG_GET_SYSTEM_STATUS_CRC				10		 // gets the parameter buffer crc
...
// MANDOLIN_MSG_NESTED
// Defines
#define MANDOLIN_NESTED_TARGETID_NONE 0
#define MANDOLIN_NESTED_TARGETID_BROOKLYN 1	
#define MANDOLIN_NESTED_TARGETID_GOVN 2
#define MANDOLIN_NESTED_TARGETID_DSP1 3
#define MANDOLIN_NESTED_TARGETID_DSP2 4

// MANDOLIN_MSG_CUSTOM
typedef enum {
MANDOLIN_CUSTOM_MAKE_FACTORY = 1,			// no args
MANDOLIN_CUSTOM_ECHO,						// echo test, sequence #, n data
MANDOLIN_CUSTOM_FLASH_WRITE,				// addr,length values[i],
MANDOLIN_CUSTOM_FLASH_ERASE,				// addr,length OR just addr for status
MANDOLIN_CUSTOM_FLASH_VERIFY,
MANDOLIN_CUSTOM_LINUX_COMMAND,
MANDOLIN_CUSTOM_MICSWEEP,
MANDOLIN_CUSTOM_MEMORY_READ,				// addr, length
MANDOLIN_CUSTOM_MEMORY_WRITE,				// addr, data, data, etc
MANDOLIN_CUSTOM_IRDA,
MANDOLIN_CUSTOM_DIAGNOSTIC,
MANDOLIN_CUSTOM_TIME,						// set and get time functions
MANDOLIN_CUSTOM_TILTSENSOR,	
MANDOLIN_CUSTOM_GETHARDWAREINFODETAILS,	
MANDOLIN_CUSTOM_SETMODELTYPE,
MANDOLIN_CUSTOM_UPDATEFROMLOCALFILE,			// upgrading from a local file	

} mandolin_custom_id;

typedef enum {
MANDOLIN_CUSTOM_MICSWEEP_CALIBRATE_START = 1,	//  nChannel (0 for all), fLevel, nBurstLength, return ok/no
MANDOLIN_CUSTOM_MICSWEEP_CALIBRATE_PROGRESS,	// no args, returns  no or yes (array of chan/sucess),OR  busy with percent  
MANDOLIN_CUSTOM_MICSWEEP_CALIBRATE_STOP,		// no args, returns  no, yes no or yes (array of chan/sucess)
MANDOLIN_CUSTOM_MICSWEEP_SINGLE_START,		// channel # (1..22), fLevel, nBurstLength,  returns ok/no
...
MANDOLIN_CUSTOM_MICSWEEP_DSP_START,		// channel 1..22, returns -1 if error, 0 not handled, 1 if handled
...
} mandolin_custom_micsweep_id;

typedef enum {
MANDOLIN_CUSTOM_IRDA_NEIGHBOR_LIST_RESET = 1,	// args: none, response: ack
MANDOLIN_CUSTOM_IRDA_NEIGHBOR_LIST_GET,		// none,  returns list: top,bottom,left,right,count top, count bottom, count left, count right
MANDOLIN_CUSTOM_IRDA_NEIGHBORTRANSMIT_HARDWAREINFO, // transithardware info, args: pos (-1 for all, 0,1,2,3) returns: succuess, busy with %
MANDOLIN_CUSTOM_IRDA_NEIGHBORREQUEST_HARDWAREINFO,	// requests all neighbors for info: args: pos (-1 for all, 0,1,2,3)
MANDOLIN_CUSTOM_IRDA_INTERROGATE_NEIGHBOR,	// pos, own hardware info,  return:  pos, hw info, requester pos, requester dest hw info
MANDOLIN_CUSTOM_IRDA_SENDECHO,	// pos, variable data (or no data to get progress) return: fail,pass or busy with %
MANDOLIN_CUSTOM_IRDA_ECHO,	//  pos, own hardware info, variable data   return: pos, hw info, requester pos, requester dest hw info, ones complement of data  (used in testing)
MANDOLIN_CUSTOM_IRDA_SENDHARDWAREINFO,	//  pos, own hardware info  return: NO reply
} mandolin_custom_irda_id;

typedef enum {
MANDOLIN_CUSTOM_TILTSENSOR_GET = 1,	// args: none, response: X,Y,TARE_X,TARE_Y
MANDOLIN_CUSTOM_TILTSENSOR_CALIBRATE,	// args: 0(all),1=x,2=y,3=z, response: X,Y,TARE_X,TARE_Y
MANDOLIN_CUSTOM_TILTSENSOR_SET, //X,Y
} mandolin_custom_tiltsensor_id;


#define MANDOLIN_CUSTOM_TILTSENSORTYPE_CMA3000 0
#define  MANDOLIN_CUSTOM_TILTSENSORTYPE_ADXL345 1


typedef enum {
MANDOLIN_CUSTOM_GETHARDWAREINFODETAILS_DSP = 1, // args:none: response: 
MANDOLIN_CUSTOM_GETHARDWAREINFODETAILS_GOV,  // args:none: response: 
} mandolin_custom_gethwdetails_id;

#define MANDOLIN_CUSTOM_TILTSENSOR_POS_ALL	0
#define MANDOLIN_CUSTOM_TILTSENSOR_POS_X	1
#define MANDOLIN_CUSTOM_TILTSENSOR_POS_Y	2
#define MANDOLIN_CUSTOM_TILTSENSOR_POS_Z	3

// MANDOLIN_CUSTOM_DIAGNOSTIC
typedef enum {
MANDOLIN_CUSTOM_DIAGNOSTIC_CHAN_FAULT_FAST = 1, // no args. Returns Amp_Fault flags, or empty if unsupported channel
} mandolin_custom_diagnostic_id;

#define MANDOLIN_CUSTOM_DIAG_BITS_AMPPROTECT		0x01
#define MANDOLIN_CUSTOM_DIAG_BITS_AMPIMPEDANCEFAULT 0x02
#define MANDOLIN_CUSTOM_DIAG_BITS_LIMITERON			0x04
#define MANDOLIN_CUSTOM_DIAG_BITS_AMPMUTEON			0x08
#define MANDOLIN_CUSTOM_DIAG_BITS_TEMPERATUREFAULT	0x10
#define MANDOLIN_CUSTOM_DIAG_MASK_ERROR				(MANDOLIN_CUSTOM_DIAG_BITS_AMPPROTECT+MANDOLIN_CUSTOM_DIAG_BITS_AMPIMPEDANCEFAULT+MANDOLIN_CUSTOM_DIAG_BITS_AMPMUTEON+MANDOLIN_CUSTOM_DIAG_BITS_TEMPERATUREFAULT)
#define MANDOLIN_CUSTOM_DIAG_MASK_WARNING			(MANDOLIN_CUSTOM_DIAG_BITS_LIMITERON)

// MANDOLIN_CUSTOM_LED_OPTIONS
#define MANDOLIN_CUSTOM_LED_OPTIONS_GLOBAL     0x01
#define MANDOLIN_CUSTOM_LED_OPTIONS_POWER_ON   0x02
#define MANDOLIN_CUSTOM_LED_OPTIONS_IDENTIFY   0x04
#define MANDOLIN_CUSTOM_LED_OPTIONS_FAULT      0x08
#define MANDOLIN_CUSTOM_LED_OPTIONS_LIMIT      0x10
#define MANDOLIN_CUSTOM_LED_OPTIONS_CLIP       0x20


typedef enum
{
MANDOLIN_CUSTOM_TIME_SET = 1,
MANDOLIN_CUSTOM_TIME_GET,
} mandolin_custom_time_id;
...
// ID LED Mode
typedef enum {
	MANDOLIN_ID_LED_OFF = 0,
	MANDOLIN_ID_LED_ON,
	MANDOLIN_ID_LED_BLINK
} mandolin_idled_mode;
...
mandolin_fifo_type MANDOLIN_MSG_readbuffer(mandolin_byte_type **pBuffer, uint16* nLength, mandolin_message *bmp, mandolin_byte_type *payload_ptr);
